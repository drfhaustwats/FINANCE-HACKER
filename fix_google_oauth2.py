...
def fix_google_oauth():
    ...
    backup_path = Path('/app/backend/server.py.bak2')
    ...
    # Fix 1: Move OAuth client creation after environment variables are loaded
    oauth_client_code = """
...
"""
    ...
    # Find the position after environment variables are loaded
    env_vars_loaded = "db = client[os.environ['DB_NAME']]"
    if env_vars_loaded in content:
        content = content.replace(env_vars_loaded, env_vars_loaded + oauth_client_code)

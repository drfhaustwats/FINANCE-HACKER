...
def test_create_users():
    try:
        ...
        if success_a:
            ...
            print(f"Created User A: {user_a_data['email']} with ID: {user_a_id}")
        else:
            print(f"Failed to create User A: {response_a.text}")
            return print_test_result("Create Users for Data Segmentation", False, response_a)
        ...
        if success_b:
            ...
            print(f"Created User B: {user_b_data['email']} with ID: {user_b_id}")
        else:
            print(f"Failed to create User B: {response_b.text}")
            return print_test_result("Create Users for Data Segmentation", False, response_b)
        
        # Store user data for later use
        global user_a_data_obj, user_b_data_obj
        user_a_data_obj = user_a_data
        user_b_data_obj = user_b_data
        
        return print_test_result("Create Users for Data Segmentation", success_a and success_b)
    except Exception as e:
        return print_test_result("Create Users for Data Segmentation", False, error=str(e))

# Test 2: Create transactions for User A
def test_create_transactions_for_user_a():
    try:
        if not hasattr(sys.modules[__name__], 'user_a_id'):
            success = test_create_users()
            if not success:
                return print_test_result("Create Transactions for User A", False, error="Failed to get User A ID")
...
# Test 3: Create transactions for User B
def test_create_transactions_for_user_b():
    try:
        if not hasattr(sys.modules[__name__], 'user_b_id'):
            success = test_create_users()
            if not success:
                return print_test_result("Create Transactions for User B", False, error="Failed to get User B ID")
...
# Test 4: Create categories for User A
def test_create_categories_for_user_a():
    try:
        if not hasattr(sys.modules[__name__], 'user_a_id'):
            success = test_create_users()
            if not success:
                return print_test_result("Create Categories for User A", False, error="Failed to get User A ID")
...
# Test 5: Create categories for User B
def test_create_categories_for_user_b():
    try:
        if not hasattr(sys.modules[__name__], 'user_b_id'):
            success = test_create_users()
            if not success:
                return print_test_result("Create Categories for User B", False, error="Failed to get User B ID")
...
# Test 6: Test transactions data isolation
def test_transactions_data_isolation():
    try:
        # Ensure we have users and their data
        if not hasattr(sys.modules[__name__], 'user_a_id') or not hasattr(sys.modules[__name__], 'user_b_id'):
            test_create_users()
...
# Test 7: Test categories data isolation
...
# Test 8: Test monthly report analytics data isolation
...
# Test 9: Test category breakdown analytics data isolation
...
def run_all_tests():
    ...
    test_results = {
        "Create Users for Data Segmentation": test_create_users(),
